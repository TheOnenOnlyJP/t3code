...
/**
 * Reorders the visible active-thread slice while preserving the slots held by
 * threads hidden by the current project scope. `currentThreadOrder` is the
 * materialized order of every active thread, so persisting it also makes a
 * first drag durable when no preference existed yet.
 */
export function reorderThreads(
  ...
  const fromIndex = visibleThreadOrder.indexOf(draggedThreadId);
  const toIndex = visibleThreadOrder.indexOf(targetThreadId);
  if (fromIndex === -1 || toIndex === -1) {
    return state;
  }

  const nextVisibleOrder = [...visibleThreadOrder];
  const [dragged] = nextVisibleOrder.splice(fromIndex, 1);
  if (dragged === undefined) {
    return state;
  }
  nextVisibleOrder.splice(toIndex, 0, dragged);

  const visibleThreadIds = new Set(visibleThreadOrder);
  let nextVisibleIndex = 0;
  const threadOrder = currentThreadOrder.map((threadId) => {
    if (!visibleThreadIds.has(threadId)) {
      return threadId;
    }
    return nextVisibleOrder[nextVisibleIndex++] ?? threadId;
  });
  if (nextVisibleIndex !== nextVisibleOrder.length) {
    return state;
  }
...
export const useUiStateStore = create<UiStateStore>((set) => ({
  ...
  reorderThreads: (currentThreadOrder, visibleThreadOrder, draggedThreadId, targetThreadId) =>
    set((state) =>
      reorderThreads(
        state,
        currentThreadOrder,
